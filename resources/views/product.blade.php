...
@section('content')
        <div class="card container p-4">
            <div class="row g-0">
                ...
                <div class="col-md-7">
                    <div class="pb-4">
                        <h1 class="card-title">{{ $data['auction']->product->name }}</h1>
                        <p class="card-text">{{ $data['auction']->product->description }}</p>
                    </div>
                    <div class="">
                        <div id="scroller" class="" style="height: 367px; overflow-y: auto;">
                            <table class="table">
                                <tbody>
                                @foreach($data['bids'] as $key => $bid)
                                    <tr class="{{ $key == count($data['bids'])-1 ? 'bg-warning' : '' }}">
                                        <td>{{$bid->name}}</td>
                    ...
                    <div class="mt-2">
                        <form action="{{route('bid.store', ['auctionId' => $data['auction']->id])}}" method="POST" class="row">
                            @csrf
                            <div class="col-6">
                                <input
                                    type="number"
